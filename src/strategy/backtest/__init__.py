...
_LEGACY_OVERRIDES: dict[str, list[dict]] = {
    "ma_crossover": [
        {"key": "fast_period", "label": "短期均线 (Fast MA)", "type": "int",
         "default": 10, "min": 3, "max": 60, "step": 1, "widget": "slider",
         "help": "金叉快线周期"},
        {"key": "slow_period", "label": "长期均线 (Slow MA)", "type": "int",
         "default": 30, "min": 10, "max": 250, "step": 1, "widget": "slider",
         "help": "死叉慢线周期"},
        {"key": "position_size", "label": "仓位比例", "type": "float",
         "default": 0.95, "min": 0.1, "max": 1.0, "step": 0.05, "widget": "slider",
         "help": "买入时占用现金比例"},
    ],
    "factor_rebalance": [
        {"key": "rebalance_days", "label": "调仓周期 (交易日)", "type": "int",
         "default": 20, "min": 5, "max": 120, "step": 5, "widget": "slider",
         "help": "每 N 天检查一次因子值，决定是否换仓"},
        {"key": "factor_line", "label": "因子列名 (data feed 中的列名)", "type": "str",
         "default": "pe",
         "help": "data feed 含此列时按真因子模式跑；否则切换到下方'偏离 MA%'模式"},
        {"key": "buy_threshold", "label": "真因子: 买入阈值 (factor <)", "type": "float",
         "default": 15.0, "min": 0.0, "max": 100.0, "step": 0.5, "widget": "slider",
         "help": "仅在'真因子模式'生效。低 PE = 价值买入信号"},
        {"key": "sell_threshold", "label": "真因子: 卖出阈值 (factor >)", "type": "float",
         "default": 30.0, "min": 0.0, "max": 200.0, "step": 0.5, "widget": "slider",
         "help": "仅在'真因子模式'生效。高 PE = 估值偏高信号"},
        {"key": "ma_period", "label": "退化模式: MA 周期", "type": "int",
         "default": 20, "min": 5, "max": 120, "step": 5, "widget": "slider",
         "help": "退化模式下用作偏离基准的均线周期"},
        {"key": "buy_below_ma_pct", "label": "退化模式: 跌破 MA 多少% 买入", "type": "float",
         "default": -5.0, "min": -30.0, "max": 0.0, "step": 0.5, "widget": "slider",
         "help": "close 相对 MA 偏离 ≤ 此值（负数）时买入。例 -5 = 价格跌破 MA20 5% 时买"},
        {"key": "sell_above_ma_pct", "label": "退化模式: 超出 MA 多少% 卖出", "type": "float",
         "default": 8.0, "min": 0.0, "max": 50.0, "step": 0.5, "widget": "slider",
         "help": "close 相对 MA 偏离 ≥ 此值（正数）时卖出"},
        {"key": "position_size", "label": "仓位比例", "type": "float",
         "default": 0.95, "min": 0.1, "max": 1.0, "step": 0.05, "widget": "slider"},
        {"key": "strict_factor", "label": "严格模式 (缺因子列直接报错)", "type": "bool",
         "default": False,
         "help": "True 时若 data feed 不含 factor_line 则抛错，不走退化模式"},
    ],
    "rule_based": [
    ...
    "screener_rule": [
        ...
        {"key": "sell_conditions", "label": "卖出条件 (筛选器格式)", "type": "yaml",
         "default": [{"type": "rsi_overbought", "threshold": 70, "period": 14}],
         "help": "例如 RSI 超过 70 卖出"},
        {"key": "buy_logic", "label": "买入逻辑", "type": "str", "default": "any",
         "options": ["any", "all"],
        ...
        {"key": "position_size", "label": "仓位比例", "type": "float",
         "default": 0.95, "min": 0.1, "max": 1.0, "step": 0.05, "widget": "slider"},
        {"key": "max_lookback", "label": "条件回溯窗口 (K 线根数)", "type": "int",
         "default": 300, "min": 60, "max": 500, "step": 20, "widget": "slider",
         "help": "条件计算时回溯的最大 K 线数（MA250 需 ≥250）"},
    ],
    "ml_rebalance": [
        {"key": "rebalance_days", "label": "调仓周期 (天)", "type": "int",
         "default": 20, "min": 5, "max": 60, "step": 1, "widget": "slider",
         "help": "ML 模型预测的标签是未来 20 日超额收益，建议保持一致"},
        {"key": "buy_threshold", "label": "买入阈值 (预测超额% ≥)", "type": "float",
         "default": 0.0, "min": -2.0, "max": 5.0, "step": 0.1, "widget": "slider",
         "help": "预测未来超额收益 ≥ 此值时建仓。多数模型 IC 约 0.05，预测分常在 ±0.5% 范围；过高会导致 0 交易"},
        {"key": "sell_threshold", "label": "卖出阈值 (预测超额% ≤)", "type": "float",
         "default": -0.5, "min": -5.0, "max": 2.0, "step": 0.1, "widget": "slider",
         "help": "预测未来超额收益 ≤ 此值时清仓"},
        {"key": "position_size", "label": "仓位比例", "type": "float",
         "default": 0.95, "min": 0.1, "max": 1.0, "step": 0.05, "widget": "slider"},
        {"key": "warmup_bars", "label": "预热 K 线数", "type": "int",
         "default": 250, "min": 30, "max": 500, "step": 10, "widget": "slider",
