...
import threading, os, sys, contextlib
...
def _bs_release() -> None:
    ...
    with _bs_session_lock:
        ...
        if _bs_ref_count == 0 and _bs_logged_in:
            try:
                with _bs_lock, _suppress_stdout(): bs.logout()
            except: pass
            finally: _bs_logged_in = False
...
def to_bs_code(code: str | None) -> str:
    ...
    if len(code) != 6 or not code.isdigit(): return code
...
class BaostockProvider:
    def __init__(self): self._acquired = False
    def login(self):
        if not self._acquired and _bs_acquire(): self._acquired = True
        return self
    def logout(self):
        if self._acquired: _bs_release(); self._acquired = False
    def __enter__(self): return self.login()
    def __exit__(self, t, v, tb): self.logout()
        ...
        with _bs_lock:
            if not _bs_logged_in:
                with _suppress_stdout():
                    lg = bs.login()
                    if lg.error_code == "0": _bs_logged_in = True
    ...
    def get_k_data(self, code, days_back=120, frequency="d", **kwargs) -> pd.DataFrame:
        ...
        if len(bs_code) != 9: return pd.DataFrame()
        ...
        fields = kwargs.get("fields") or ("date,open,high,low,close,volume,amount,turn,peTTM,pbMRQ,psTTM,isST" if frequency == "d" else "date,open,high,low,close,volume,amount,turn,pctChg")
        df = self._safe_query(bs.query_history_k_data_plus, code=bs_code, fields=fields, 
                             start_date=start, end_date=end, frequency=frequency, adjustflag=kwargs.get("adjust", "2"))
        if not df.empty:
    ...
    def get_profit_data(self, code, year=0, quarter=0):
        c = to_bs_code(code)
        if len(c) != 9: return pd.DataFrame()
        if year == 0:
            now = datetime.now()
    ...
    def get_latest_price(self, code):
        ...
        if df.empty: return {"price": 0.0}
        l = df.iloc[-1]
        return {
            "price": float(l.get("close", 0) or 0),
            "peTTM": float(l.get("peTTM", 0) or 0),
            "pbMRQ": float(l.get("pbMRQ", 0) or 0),
            "psTTM": float(l.get("psTTM", 0) or 0),
            "turn": float(l.get("turn", 0) or 0),
